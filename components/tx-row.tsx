      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <span className={`text-xs px-3 py-1 rounded-full inline-block ${tx.end_date ? 'line-through' : ''}`} style={{ backgroundColor: tx.type === 'expense' ? '#FF4D8A33' : tx.type === 'income' ? '#6A7BFF33' : '#2CE6D633', color: tx.type === 'expense' ? '#FF4D8A' : tx.type === 'income' ? '#6A7BFF' : '#2CE6D6' }}>
              {!cat
