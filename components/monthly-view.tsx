    ...
    <div>
      <div className="grid grid-cols-7 pt-3 pb-0 px-0">
        {WEEKDAYS_MON.map(day => (
          <div key={day} className="text-center text-[10px] font-medium text-gray-500 dark:text-gray-300">{day}</div>
        ))}
      </div>
...
const WeekDayCard = memo(function WeekDayCard({
  ...
  return (
    <div
      ref={registerRef}
      className={`mx-5 mb-3 rounded-[22px] bg-surface dark:bg-gray-900 transition-colors ${highlighted ? 'ring-1 ring-accent-blue/40' : ''}`}
    >
      ...
      <div className="mx-4 border-t border-gray-300 dark:border-gray-500" />
      ...
      <div className="px-4 pb-5 pt-3">
              ...
              >
                <div className="min-w-0 flex flex-1 items-center gap-3 overflow-hidden">
                  ...
                  <div className="min-w-0 flex flex-1 items-center gap-3 overflow-hidden text-[14px] text-foreground">
                    ...
                    {memoText && <span className="truncate text-[10px] text-gray-500 dark:text-gray-300">{memoText}</span>}
                  </div>
                </div>
            ...
            >
              <div className="min-w-0 flex flex-1 items-center gap-3 overflow-hidden">
                ...
                <div className="min-w-0 flex flex-1 items-center gap-3 overflow-hidden text-[14px] text-foreground">
                  ...
                  {r.description && <span className="truncate text-[10px] text-gray-500 dark:text-gray-300">{r.description}</span>}
                </div>
              </div>
        ...
        <div className="mt-3 border-t border-gray-300 dark:border-gray-500 pt-3 space-y-2">
          <div className="flex items-center justify-between text-[14px] text-gray-500 dark:text-gray-300">
            <span className="font-medium text-gray-500 dark:text-gray-300">수입</span>
            <span className="font-semibold tracking-[-0.02em] tabular-nums text-gray-500 dark:text-gray-300">₩{dayIncome.toLocaleString()}</span>
          </div>
          <div className="flex items-center justify-between text-[14px] text-gray-500 dark:text-gray-300">
            <span className="font-medium text-gray-500 dark:text-gray-300">지출</span>
            <span className="font-semibold tracking-[-0.02em] tabular-nums text-gray-500 dark:text-gray-300">₩{dayExpense.toLocaleString()}</span>
          </div>
        </div>
      </div>
    </div>
      ...
      {viewMode === 'week' && (
        <>
          <div className="mx-5 mb-3 mt-2 flex gap-3">
            <div className="flex-1 bg-surface dark:bg-gray-900 rounded-[22px] px-4 pt-4 pb-3">
