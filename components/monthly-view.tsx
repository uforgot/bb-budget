      ...
      <div className="grid grid-cols-7 gap-2">
        ...
        {weekDays.map(day => {
          ...
          return (
            ...
            >
              <span className={`text-[10px] font-medium ${selected ? 'text-white/80' : 'text-gray-500 dark:text-gray-500'}`}>{weekdayLabel}</span>
            ...
            <div className="flex gap-2" style={{ width: 'max-content' }}>
              {Array.from({ length: totalWeeks }, (_, i) => i + 1).map(week => (
                <button
                  ...
                  className={`px-6 py-2 rounded-full text-[14px] font-semibold whitespace-nowrap transition-colors ${viewMode === 'week' && selectedWeek === week ? 'bg-accent-blue text-white' : 'bg-gray-100 dark:bg-gray-800 text-gray-400 dark:text-gray-500'}`}
                >
                  {week}주 차
                </button>
              ))}
