...
export function AddTransactionHeader({
  ...
  return (
    <header className="flex items-center justify-between px-5 pt-[env(safe-area-inset-top,0px)] h-16 bg-[#eceff3] dark:bg-surface flex-shrink-0">
      <button
        onClick={onClose}
          ...
          {(['지출', '수입', '저축'] as TransactionType[]).map((t) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-[14px] font-medium transition-colors ${
                type === t ? typeColors[t].active : 'bg-[#e5e7eb] dark:bg-surface text-muted-foreground'
              }`}
            >
              {t}
            </button>
