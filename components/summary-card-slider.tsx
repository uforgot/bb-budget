...
export function SummaryCardSlider({
  ...
  prevMonth, prevIncome, prevExpense, prevSavings, prevBalance, hasPrev, yearMode,
}: SummaryCardSliderProps) {
  const unit = yearMode ? '년' : '월'
  const prevLabel = `${prevMonth}${unit}`
  ...
  const cards = [
    { label: `${month}${unit} 번 수입`, amount: income, diff: hasPrev ? income - prevIncome : null, type: 'income' as const, textColor: 'text-white', bg: '#6366F1', img: '/card-income.png' },
    { label: `${month}${unit} 쓴 지출`, amount: expense, diff: hasPrev ? expense - prevExpense : null, type: 'expense' as const, textColor: 'text-white', bg: '#EC4899', img: '/card-expense.png' },
    { label: `${month}${unit} 모은 저축`, amount: savings, diff: hasPrev ? savings - prevSavings : null, type: 'savings' as const, textColor: 'text-white', bg: '#7C3AED', img: '/card-saving.png' },
