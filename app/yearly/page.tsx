...
export default function Yearly() {
  ...
  const yearIncome = transactions.filter(t => t.type === 'income' && t.date <= compareEnd && parseDateParts(t.date).year === targetYear).reduce((s, t) => s + t.amount, 0)
  const yearExpense = transactions.filter(t => t.type === 'expense' && t.date <= compareEnd && parseDateParts(t.date).year === targetYear).reduce((s, t) => s + t.amount, 0)
  const yearSavings = transactions.filter(t => t.type === 'savings' && t.date <= compareEnd && (!t.end_date || t.end_date > compareEnd)).reduce((s, t) => s + t.amount, 0)
      ...
      {!searchMode && (
        <>
          <SummaryCardSlider
            ...
            hasPrev={hasPrevYear}
            yearMode
            labelPrefixOverride={targetYear === today.getFullYear() ? `${targetYear}년` : undefined}
            prevLabelOverride={targetYear === today.getFullYear() ? `${prevYear}년 동일 시점` : undefined}
          />
            ...
            <div className="mb-0">
              <div className="flex gap-0 overflow-visible">
                {([
                  ...
                  const inactiveClass = distance === 1 ? 'bg-surface/60 text-gray-300 dark:text-gray-600' : 'bg-surface/30 text-gray-300 dark:text-gray-600'
                  return (
                    <button
                      key={key}
                      onClick={() => setYearlyChartMode(key)}
                      className={`relative flex-1 h-10 rounded-t-[22px] rounded-b-none text-[13px] font-semibold transition-colors ${active ? 'z-20 bg-surface dark:bg-gray-900 text-foreground' : `z-10 ${inactiveClass}`}`}
                    >
