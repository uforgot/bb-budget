      ...
      <div className="sticky top-0 z-30 bg-background px-5">
        <div className="flex items-center justify-between h-14" style={{ paddingTop: 'env(safe-area-inset-top, 0px)' }}>
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => { setSearchMode(v => !v); setSearchQuery('') }} className={`flex items-center justify-center w-8 h-8 rounded-lg ${searchMode ? 'text-foreground' : 'text-muted-foreground'}`}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8" /><path d="m21 21-4.3-4.3" /></svg>
            </button>
            <button onClick={() => router.push('/settings')} className="flex items-center justify-center w-8 h-8 rounded-lg text-muted-foreground">
