              ...
              <>
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={monthlyAssets} margin={{ left: 0, right: 0, top: 8, bottom: 0 }}>
                      ...
                      content={({ active, payload, label }) => {
                        ...
                        return (
                          <div className="bg-background rounded-lg px-3 py-2">
                            ...
                            <p className="text-xs" style={{ color: '#5D00FF' }}>잔액 {fmt(cash)}</p>
                    ...
                    />
                    <Bar dataKey="cash" stackId="a" fill="#5D00FF" radius={[0, 0, 0, 0]} />
                ...
                <div className="flex gap-4 mt-3">
                  <div className="flex items-center gap-1.5">
                    <div className="w-3 h-3 rounded-full" style={{ backgroundColor: '#5D00FF' }} />
          ...
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={yearlyData} margin={{ left: 10, right: 10, top: 8, bottom: 0 }}>
              ...
              {(trendMode === 'expense' || trendMode === 'all') && (
                <Line type="monotone" dataKey="expense" stroke="#FF2E9A" strokeWidth={2} dot={{ r: 3, fill: '#FF2E9A' }} connectNulls />
              )}
              {(trendMode === 'income' || trendMode === 'all') && (
                <Line type="monotone" dataKey="income" stroke="#5D00FF" strokeWidth={2} dot={{ r: 3, fill: '#5D00FF' }} connectNulls />
              )}
            </LineChart>
          </ResponsiveContainer>
        </Card>
                ...
                {top3.map((cat, i) => (
                  <div key={cat.catId} className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="text-[12px] bg-muted px-2 py-0.5 rounded-full">{cat.name}</span>
