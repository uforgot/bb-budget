...
4. **달력 가상 스크롤 전환** — 현재 단순 grid+화살표 → `react-window` 기반 무한 세로 스크롤로 교체. pink-tracker Calendar.tsx 참고 (MonthGrid, List, ±12개월 초기 로드, 가장자리 동적 확장, 포커스 월 opacity). 스와이프만 추가하는 건 UX 별로 → 가상 스크롤 필수
